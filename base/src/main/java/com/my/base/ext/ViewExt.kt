 ...
 * 防止重复点击,可同时注册多个view
 */
fun setNoRepeatClick(vararg views: View, interval: Long = 1000, onClick: (View) -> Unit) {
    views.forEach {
 ...
 */
var lastTime = 0L
fun View.clickNoRepeat(interval: Long = 1000, onClick: (View) -> Unit) {
    setOnClickListener {
 ...
 */
fun copy(msg: String) {
    val clip = ContextUtils.getContext().getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
    clip.text = msg
    toast("已复制")
}
...
fun SmartRefreshLayout.enableRefreshAndLoadMore() {
...
fun px2dp(context: Context, dpValue: Float): Int {
    val scale: Float = ContextUtils.getContext().resources.displayMetrics.density
    return (dpValue / scale + 0.5f).toInt()
}

fun px2sp(pxValue: Float): Int {
    val fontScale: Float = ContextUtils.getContext().resources.displayMetrics.scaledDensity
